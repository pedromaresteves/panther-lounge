...
function pageAction(e){
  ...
  let httpRequest;
  const apiUrl = e.target.href.replace("guitar-chords", "api");
  ...
  httpRequest.onreadystatechange = function(){    
      ...
      if(httpRequest.readyState === 4){
        ...
        if(response.length == 1){
          ...
          response[0].forEach(function(item){
            newResultsHtml += `<li class="list-group-item artist-item"><a href="${response[1]}/${item.link}">${item.title}</a><span><a href="edit-song/${response[1]}/${item.link}" class="mr-3">Edit</a><a href="/" data-toggle="modal" data-target="#modal-${item.title}">Delete</a></span></li>
            <div class="modal fade" id="modal-${item.title}" tabindex="-1" role="dialog" aria-labelledby="${item.link}-label" aria-hidden="true">
